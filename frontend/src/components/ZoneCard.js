...
export default function ZoneCard({ zone, onPress }) {
  const getZoneDesignProps = (typeZone) => {
    ...
    if (['boutique', 'librairie', 'cafe_majorelle', 'cafe_bousafsaf'].includes(typeZone)) {
      return { type: 'COMMERCIAL', typeColor: COLORS.commercialBg, typeTextColor: COLORS.commercial, fallback: require('../../assets/majorelle_museum.png') };
    }
  ...
  const cardImage = zone.image ? { uri: zone.image } : props.fallback;
  ...
  return (
    <TouchableOpacity style={styles.card} onPress={onPress}>
      ...
      <View style={styles.cardInfo}>
        ...
        <Text style={styles.cardTitle} numberOfLines={1}>{zone.nom}</Text>
        <Text style={styles.cardDesc} numberOfLines={2}>{zone.description}</Text>
      </View>
    </TouchableOpacity>
  );
}
